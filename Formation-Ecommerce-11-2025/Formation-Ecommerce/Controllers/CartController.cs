...
{
    /// <summary>
    /// Contrôleur MVC du panier : affichage du contenu, application/suppression de coupon, suppression d'articles et passage de commande.
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Couche Web : le contrôleur ne fait pas d'accès direct base de données, il délègue à des services applicatifs.
    /// - <see cref="ICartService"/> centralise la logique de panier (lecture, upsert, suppression, nettoyage).
    /// - <see cref="ICouponService"/> sert ici à valider un code coupon avant de l'appliquer au panier.
    /// - <see cref="IOrderServices"/> est appelé au checkout pour persister la commande (header + détails).
    /// - <see cref="UserManager{TUser}"/> fournit l'identité de l'utilisateur connecté afin d'isoler les paniers par utilisateur.
    /// </remarks>

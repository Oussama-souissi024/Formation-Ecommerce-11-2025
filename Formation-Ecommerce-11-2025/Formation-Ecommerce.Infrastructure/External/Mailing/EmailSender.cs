...
{
    /// <summary>
    /// Service d'envoi d'emails (SMTP) : envoi générique + emails applicatifs (confirmation, reset password, bienvenue).
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Couche Infrastructure : implémentation technique de <see cref="IEmailSender"/> basée sur MailKit.
    /// - La configuration SMTP est injectée via <see cref="IOptions{TOptions}"/> (appsettings.json).
    /// - Les tokens (confirmation/reset) doivent être encodés dans l'URL, d'où l'usage de UrlEncode.
    /// </remarks>
    public class EmailSender : IEmailSender
    {
            ...
            public async Task SendEmailConfirmationAsync(string email, string confirmationToken, string userId)
        {
            ...
            var confirmationLink = $"https://localhost:7238/Auth/ConfirmEmail?token={encodedToken}&userId={userId}";
        ...
        public async Task SendPasswordResetEmailAsync(string email, string resetToken, string userId)
        {
            ...
            var resetLink = $"https://localhost:7238/Auth/ResetPassword?token={encodedToken}&userId={userId}";
            var message = new EmailMessage
            {
                To = email,
                ...
                Body = $@"
                    ...
                    <body style=""font-family: Arial, sans-serif; line-height: 1.6;"">
                        <div style=""max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e4e4e4; border-radius: 5px;"">
                            ...
                            <div style=""text-align: center; margin: 30px 0;"">
                                <a href='https://localhost:7238/' style=""display: inline-block; background-color: #4CAF50; color: white; padding: 12px 20px; text-decoration: none; border-radius: 4px;"">

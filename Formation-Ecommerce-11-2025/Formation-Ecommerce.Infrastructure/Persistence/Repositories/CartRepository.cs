...
{
    /// <summary>
    /// Repository EF Core du panier : accès aux données du panier (header + détails) et opérations spécifiques (clear, total count).
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Le panier est composé de deux tables liées : CartHeader (1) -> CartDetails (n).
    /// - Les méthodes utilisent <see cref="Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.Include"/> pour charger les produits associés.
    /// - Le repository orchestre le DbContext et des repositories génériques afin de maîtriser le moment du SaveChanges.
    /// </remarks>

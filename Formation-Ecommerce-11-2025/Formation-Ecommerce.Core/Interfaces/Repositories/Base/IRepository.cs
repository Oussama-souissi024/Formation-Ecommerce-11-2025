...
namespace Formation_Ecommerce_11_2025.Core.Interfaces.Repositories.Base
{
    ...
    /// <summary>
    /// Contrat générique de repository : définit les opérations CRUD de base pour une entité.
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - L'objectif est de découpler la logique métier (Application) de la technologie de persistance (EF Core, Dapper, etc.).
    /// - Le type générique <typeparamref name="TEntity"/> représente l'entité manipulée.
    /// - Les méthodes sont asynchrones pour ne pas bloquer le thread lors des I/O (base de données).
    /// </remarks>

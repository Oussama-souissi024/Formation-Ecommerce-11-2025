...
/// - Fournir une implémentation simple pour les tests unitaires de <c>ProductServices</c> et <c>CartService</c>.
...
public class FakeProductRepository : IProductRepository
{
    ...
    public Task<Product> AddAsync(Product product)
    {
        ...
        {
            product.Category = new Category { Id = product.CategoryId, Name = string.Empty, Description = string.Empty, Products = new List<Product>() };
        }

        if (product.CartDetails == null)
        {
    ...
    public Task<IEnumerable<Product>> ReadAllAsync() => Task.FromResult<IEnumerable<Product>>(_store.Values.ToList());
    ...
    Task<Product> Formation_Ecommerce_11_2025.Core.Interfaces.Repositories.Base.IRepository<Product>.AddAsync(Product entity) => AddAsync(entity);

    public Task<Product> GetByIdAsync(Guid id) => ReadByIdAsync(id);

    public Task<IEnumerable<Product>> GetAllAsync() => ReadAllAsync();

    Task Formation_Ecommerce_11_2025.Core.Interfaces.Repositories.Base.IRepository<Product>.Update(Product entity) => UpdateAsync(entity);

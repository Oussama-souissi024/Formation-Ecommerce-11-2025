...
{
    /// <summary>
    /// Service applicatif des commandes : création de commande (header + détails), consultation et mise à jour de statut.
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Couche Application : centralise l'orchestration des cas d'usage "Order" et évite de mettre ces règles dans les contrôleurs.
    /// - <see cref="IOrderRepository"/> fournit la persistance (EF Core) et les requêtes spécialisées (ex: get with details, update status).
    /// - AutoMapper gère la transformation entre entités Core (<see cref="OrderHeader"/>, <see cref="OrderDetails"/>) et DTOs.
    /// </remarks>

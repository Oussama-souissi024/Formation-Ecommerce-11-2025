...
{
    /// <summary>
    /// Contrôleur MVC des commandes : consultation des commandes, détails d'une commande et mise à jour de statut (workflow côté admin).
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Couche Web : ce contrôleur expose des actions MVC (Views + endpoints JSON) et délègue les règles applicatives à <see cref="IOrderServices"/>.
    /// - L'autorisation est gérée via <see cref="AuthorizeAttribute"/> (ex: actions réservées à l'Admin) et via l'identité (claims/roles).
    /// - <see cref="UserManager{TUser}"/> et les Claims servent à récupérer l'utilisateur courant pour filtrer les commandes côté serveur.
    /// - Les statuts sont centralisés dans <see cref="StaticDetails"/> pour éviter les chaînes "magiques".
    /// </remarks>

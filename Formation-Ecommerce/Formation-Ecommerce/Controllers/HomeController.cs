...
{
    /// <summary>
    /// Contrôleur MVC de la page d'accueil (catalogue) : affiche la liste des produits et gère l'ajout d'un produit au panier.
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Le contrôleur reste "mince" : il orchestre les appels aux services (Application layer) et prépare les ViewModels.
    /// - <see cref="IProductServices"/> fournit la lecture des produits (DTOs), puis AutoMapper transforme en ViewModels.
    /// - <see cref="ICartService"/> est utilisé pour créer/mettre à jour le panier (Upsert) à partir de l'utilisateur connecté.
    /// - <see cref="UserManager{TUser}"/> sert à récupérer l'identifiant de l'utilisateur courant côté serveur.
    /// </remarks>

...
{
    /// <summary>
    /// Contrôleur MVC d'authentification : inscription, connexion, déconnexion, confirmation d'email et réinitialisation du mot de passe.
    /// </summary>
    /// <remarks>
    /// Points pédagogiques :
    /// - Couche Web (MVC) : reçoit la requête HTTP, valide le modèle, puis délègue la logique métier à <see cref="IAuthService"/>.
    /// - L'envoi d'emails (confirmation / bienvenue / reset password) est externalisé via <see cref="IEmailSender"/>.
    /// - AutoMapper mappe les ViewModels (UI) vers des DTOs (Application layer), afin d'éviter de coupler l'UI au domaine.
    /// </remarks>
